...
public protocol ShopProtocol {
  ...
  func transferFrom(contractAddress: EthereumAddress , from: EthereumAddress, to: EthereumAddress, amount: BigUInt ,from: EthereumAddress, gasPrice: BigUInt) async throws -> EthereumTransaction
}
...
open class Shop: ShopProtocol {
  ...
  public func transferFrom(contractAddress: EthereumAddress , from: EthereumAddress, to: EthereumAddress, amount: BigUInt, from: EthereumAddress, gasPrice: BigUInt) async throws -> EthereumTransaction {
    let tryCall = ShopFunctions.transferFrom(contract: contractAddress, from: from, gasPrice: gasPrice, from: from, to: to, amount: amount)
    let subdata = try tryCall.transaction()
    let gas = try await client.eth_estimateGas(subdata)
    let function = ShopFunctions.transferFrom(contract: contractAddress, from: from, gasPrice: gasPrice,gasLimit: gas, from: from, to: to, amount: amount)
    let data = try function.transaction()
    return data
}
...
open class ShopContract {
  ...
  public func totalSupply() async throws -> BigUInt{
    ...
    public func transferFrom(from: EthereumAddress,to: EthereumAddress,amount: BigUInt, account: EthereumAccount) async throws -> String{
      let gasPrice = try await client.eth_gasPrice()
      let transaction = try await (ShopCall?.transferFrom(contractAddress:contract,from: from,to: to,amount: amount, from: account.address, gasPrice: gasPrice))!
      let txHash = try await client.eth_sendRawTransaction(transaction, withAccount: account)
      return txHash
   }
